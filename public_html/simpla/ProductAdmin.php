	...
	public function fetch()
	{
   	    		...
   	    		if($product->id)
   	    		{
		  		    ...
		  		    if(is_array($variants))
		  		    {
		  		    		...
		  		    		foreach ($prices as &$price) {
		  		    			if(!empty($price->id))
		  		    			{
		  		    				if($price->from_amount > 0)
		  		    					$this->prices->update_variant_price($price->id, $price);
		  		    				else
		  		    					$this->prices->delete_variant_price($price->id);
		  		    			}
		  		    			else
		  		    			{
									// Можно добавить только к уже имеющемуся варианту
									// $price->variant_id = $variant->id;
		  		    				if(!empty($price->variant_id))
		  		    					$price->id = $this->prices->add_variant_price($price);
		  		    			}
						...
						foreach($variants as $index=>&$variant)
						{
							...
							if ($new_variant || empty($prices))
							{
								$new_var_price = new stdClass;

								$new_var_price->price = $variant->price;
								if(!is_null($variant->compare_price))
									$new_var_price->compare_price = $variant->compare_price;
								else
									$new_var_price->compare_price = 0;
								$new_var_price->from_amount = 1;
								$new_var_price->variant_id = $variant->id;

								$prices_ids[] = $this->prices->add_variant_price($new_var_price);
							}

								// Удалить непереданные цены
							foreach ($variant->prices as $vp) {
								if(!in_array($vp->id, $prices_ids))
									$this->prices->delete_variant_price($vp->id);
							}

							$variant = $this->variants->get_variant($variant->id);

							$variant->prices = $this->prices->get_variant_prices($variant->id);
		...
		else
		{
			...
			if($product)
			{
				...
				// Цены вариантов товара
				foreach ($variants as &$v) {
					$v->prices = $this->prices->get_variant_prices($v->id);
				}
