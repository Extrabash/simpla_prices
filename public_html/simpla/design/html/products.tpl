...
<form method="get">
	<div id="search">
		...
		<input type="text" name="keyword" value="{$keyword|escape}" class="search" />
...
<div id="header">
	{if $products_count}
		{if $category->name || $brand->name}
			<h1>{$category->name|escape} {$brand->name|escape} ({$products_count} {$products_count|plural:'товар':'товаров':'товара'})</h1>
		{elseif $keyword}
	...
	<a href="{url module=ProductAdmin return=$smarty.server.REQUEST_URI}" class="add">Добавить товар</a>
</div>
		...
		<form id="list_form" method="post">
			...
			<div id="list">
				{foreach $products as $product}
					<div class="{if !$product->visible}invisible{/if} {if $product->featured}featured{/if} {if $product->to_yandex}yandex{/if} {if $product->new}new{/if} row">
						...
						<div class="image cell">
							...
							{if $image}
								<a href="{url module=ProductAdmin id=$product->id return=$smarty.server.REQUEST_URI}">
									<img src="{$image->filename|escape|resize:32:32}" />
								</a>
							{/if}
						</div>
							...
							<div class="variants">
								<ul>
									{foreach $product->variants as $variant}
										<li {if !$variant@first}class="variant" style="display:none;"{/if}>
											...
											{/if}
											<br/>
											<i title="Наличие {$product->name|escape}">Наличие {$settings->units}</i>
								...
								{if $variants_num>1}
									<div class="expand_variant">
										<a href="#" class="dash_link expand_variant">{$variants_num} {$variants_num|plural:'вариант':'вариантов':'варианта'} ↓</a>
										<a href="#" class="dash_link roll_up_variant" style="display:none;">{$variants_num} {$variants_num|plural:'вариант':'вариантов':'варианта'} ↑</a>
									</div>
								{/if}
							</div>
						...
						<div class="icons cell">
							<a href="../products/{$product->url}" class="preview" title="Предпросмотр в новом окне" target="_blank"></a>
							<a href="#" class="enable" title="Активен"></a>
							<a href="#" class="featured" title="Рекомендуемый"></a>
							<a href="#" class="yandex"  title="Yandex.Market"></a>
							<a href="#" class="new" title="Новинка"></a>
							<a href="#" class="duplicate" title="Дублировать"></a>
			...
			<div id="action">
				...
				<span id="select">
					<select name="action">
						...
						<option value="set_yandex">Отправлять в Yandex</option>
                        
                        <option value="unset_yandex">Не отправлять в Yandex</option>
						...
						{function name=category_select level=0}
							{foreach $categories as $category}
								<option value="{$category->id}">
									{section sp $level}&nbsp;&nbsp;&nbsp;{/section}{$category->name|escape}
								</option>
				...
				<input type="submit" value="Применить" id="apply_action" class="button_green" />
			</div>
		</form>
...
<div id="right_menu">
	<ul>
		...
		<li {if $filter=='new'}class="selected"{/if}><a href="{url keyword=null brand_id=null category_id=null page=null filter='new'}">Новинка</a></li>
		...
		<li {if $filter=='yandex'}class="selected"{/if}><a href="{url keyword=null brand_id=null category_id=null page=null filter='yandex'}">Yandex.Market</a></li>
	</ul>
		...
		{if $categories}
			<ul>
				...
				{foreach $categories as $c}
					<li category_id="{$c->id}" class="{if $category->id == $c->id}selected{else}droppable category{/if}">
						<a href="{url keyword=null brand_id=null page=null category_id={$c->id}}">{$c->name|escape}</a>
					</li>
	...
	{if $brands}
		<ul>
			...
			{foreach $brands as $b}
				<li brand_id="{$b->id}" class="{if $brand->id == $b->id}selected{else}droppable brand{/if}">
					<a href="{url keyword=null page=null brand_id=$b->id}">{$b->name|escape}</a>
				</li>
			{/foreach}
		</ul>
	{/if}
