	...
	*/
	...
	//		$in_stock_filter = $this->db->placehold('AND (SELECT 1 FROM __variants pv WHERE pv.product_id=p.id AND pv.price>0 AND (pv.stock IS NULL OR pv.stock>0) LIMIT 1) = ?', intval($filter['in_stock']));
		...
		$in_stock_filter = !empty($filter['in_stock']) ?
			$this->db->placehold(($filter['in_stock'] ? 'AND' : 'OR').' ('.($filter['in_stock'] ? 'v.stock IS NULL OR ' : '').'v.stock '.($filter['in_stock'] ? '>' : '=').' 0)') : '';
			...
			switch ($filter['sort'])
		{
			...
			break;
			case 'created':
			$order = "p.created $order_dir";
			break;
			case 'price':
				//$order = 'pv.price IS NULL, pv.price=0, pv.price';
			$order = "(SELECT -pv.price FROM __variants pv WHERE (pv.stock IS NULL OR pv.stock>0) AND p.id = pv.product_id AND pv.position=(SELECT MIN(position) FROM __variants WHERE (stock>0 OR stock IS NULL) AND product_id=p.id LIMIT 1) LIMIT 1) $order_dir";
			break;
		}

		// Только не для админки - сначала с картинками
		if($filter['images_first'])
			$order = "IF((SELECT 1 FROM __images AS i WHERE i.product_id=p.id LIMIT 1)=1, '1', '0') DESC, " . $order;
